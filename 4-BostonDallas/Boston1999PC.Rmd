---
title: "Boston 1999 Public Charities"
output:
...
```{r}
load("/home/samutavi/Documents/Coe College/Spellman 2018/RData/US1999PC.Rdata")
boston <- subset.data.frame(US1999PC, US1999PC$CITY == "BOSTON" & US1999PC$STATE == "MA")
dallas <- subset.data.frame(US1999PC, US1999PC$CITY == "DALLAS" | US1999PC$CITY == "FORT WORTH" & US1999PC$STATE == "TX")
```
Based on previous work, we have found that Boston's nonprofit sector may be highly concentrated in two or three zipcodes. I want to see if this is true for the public charities in Boston.
```{r}
zips <- aggregate(boston$TOTREV,
                  list(boston$ZIP5),
                  sum)
total <-sum(boston$TOTREV)
...
bostonzip
ggsave("boston15.jpg", width = 16, height = 9)
...
dallaszip
ggsave("dallas15.jpg", width = 16, height = 9)
